...
class PointNetSA(nn.Module):
    def __init__(self, global_feat = True, multi_task = False):
        ...
        self.bn2 = nn.BatchNorm1d(128)
        self.bn3 = nn.BatchNorm1d(1024)
        self.key = torch.nn.Conv1d(1024, 1024, 1)
        self.query = torch.nn.Conv1d(1024, 1024, 1)
        self.value = torch.nn.Conv1d(1024, 1024, 1)
        self.global_feat = global_feat
        self.multi_task = multi_task
    def forward(self, x):
        batchsize = x.size()[0]
        ...
        x = F.relu(self.bn2(self.conv2(x)))
        x = self.bn3(self.conv3(x))
        k = self.bn3(self.key(x))
        q = self.bn3(self.query(x))
        v = self.bn3(self.value(x))
        k = k.transpose(2,1)
        scores = torch.bmm(q, k)
        scores_softmax = F.softmax(scores, dim=-1)
        x = torch.bmm(scores_softmax, v)
        x = torch.max(x, 2, keepdim=True)[0]
        x = x.view(-1, 1024)
        if self.multi_task:
            return x, torch.cat([x.view(-1, 1024, 1).repeat(1, 1, n_pts), pointfeat], 1), trans
...
class PointNetCls(nn.Module):
    def __init__(self, k = 2):
        ...
        self.feat = PointNetfeat(global_feat=True, multi_task=False)
        self.fc1 = nn.Linear(1024, 512)
        self.fc2 = nn.Linear(512, 256)
